  ...
  httpClient.get(
    {
      url: `https://maps.googleapis.com/maps/api/geocode/json?${
        request.query.type
      }=${request.query.address}&key=${config.googleservice.apikey}`
    },
  ...
  httpClient.get(
    {
      ...
      },${request.query.longitude}&radius=1500&type=restaurant&key=${
  ...
  httpClient.get(
    {
      url: `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${
        request.query.input
      }&key=${config.googleservice.apikey}&sessiontoken=${request.user.user_id}`
    },
    function(error, res, body) {
      if (!error && res.statusCode === 200) {
        response.send(body);
